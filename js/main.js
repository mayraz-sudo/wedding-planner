const DEFAULT_VENUE_COMMITMENT = 450;
let state = { guests: [], vendors: [], tasks: [], attractions: [], gettingReady: [], budgetTarget: 0, venueCommitment: DEFAULT_VENUE_COMMITMENT };
...
async function loadState(){
  ...
  state.venueCommitment = await storageAdapter.getSetting('venueCommitment', DEFAULT_VENUE_COMMITMENT);
  renderAll();
}
...
document.getElementById('exportBackupBtn').addEventListener('click', ()=>{
  const backup = {
    ...
    venueCommitment: state.venueCommitment
  };
...
document.getElementById('restoreFile').addEventListener('change', async (e)=>{
  ...
  try{
    ...
    const venueCommitment = typeof data.venueCommitment === 'number' ? data.venueCommitment : DEFAULT_VENUE_COMMITMENT;
    ...
    await storageAdapter.setSetting('venueCommitment', venueCommitment);
    ...
    state.venueCommitment = venueCommitment;
    renderAll();
